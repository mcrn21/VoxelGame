...
const vec3 &Camera::getFront() const
{
    if (m_need_update & VIEW) {
        ...
        m_front = vec3(getRotationMat() * vec4(0, 0, -1, 1));
...
mat4 Camera::getRotationMat() const
{
    if (m_need_update & ROTATION) {
        m_need_update = m_need_update & ~ROTATION;
        m_rotation_mat = mat4{1.0f};
        m_rotation_mat = glm::rotate(m_rotation_mat, m_rotation.z, vec3(0, 0, 1));
        m_rotation_mat = glm::rotate(m_rotation_mat, m_rotation.y, vec3(0, 1, 0));
        m_rotation_mat = glm::rotate(m_rotation_mat, m_rotation.x, vec3(1, 0, 0));
    }
...
mat4 Camera::getViewMat() const
{
    if (m_need_update & VIEW) {
        m_need_update = m_need_update & ~VIEW;
        m_front = vec3(getRotationMat() * vec4(0, 0, -1, 1));
        m_view_mat = lookAt(m_position, m_position + m_front, m_up);
    }
